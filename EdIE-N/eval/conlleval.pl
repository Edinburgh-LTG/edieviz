...
if (not $latex) {
   ...
   if ($tokenCounter>0) {
	printf "Total accuracy: %6.2f%%\n",100*$correctTags/$tokenCounter;
	printf "ALL: TP: %d; ", $correctChunk;
  	printf "FP: %d; ", $foundGuessed - $correctChunk;
  	printf "FN: %d; ", $foundCorrect - $correctChunk;
...
if (not $latex) {
   for $i (@sortedTypes) {
      ...
      printf "%5s: ",$i;
	  printf "TP: %d; ", $correctChunk{$i};
	  printf "FP: %d; ", $foundGuessed{$i} - $correctChunk{$i};
	  printf "FN: %d; ", $foundCorrect{$i} - $correctChunk{$i};
      printf "precision: %6.2f%%; ",$precision;
      printf "recall: %6.2f%%; ",$recall;
      printf "FB1: %6.2f\n",$FB1;
